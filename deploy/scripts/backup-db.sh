...
#
# Layout:
#   /opt/powertradefx/backups/db/daily/   — kept 14 days
#   /opt/powertradefx/backups/db/weekly/  — Sunday's daily, kept 8 weeks
#   /opt/powertradefx/backups/db/monthly/ — 1st-of-month, kept 12 months
#
# Cron (3:15 AM IST as the `powertradefx` user):
#   15 3 * * * /opt/powertradefx/deploy/scripts/backup-db.sh
#
# Restore with deploy/scripts/restore-db.sh <file>.
#
# Off-server copy (optional but recommended): rclone to Hostinger Object
# Storage / S3 / Backblaze. Set RCLONE_REMOTE in /opt/powertradefx/.env and
...
DOM=$(date +%d)        # 01..31
FILE="$BACKUP_ROOT/daily/powertradefx-$STAMP.dump"
...
if [ "$DOW" = "7" ]; then
  cp -a "$FILE" "$BACKUP_ROOT/weekly/powertradefx-$STAMP.dump"
  log "promoted → weekly"
fi
if [ "$DOM" = "01" ]; then
  cp -a "$FILE" "$BACKUP_ROOT/monthly/powertradefx-$STAMP.dump"
...
find "$BACKUP_ROOT/daily"   -type f -name 'powertradefx-*.dump' -mtime +14 -delete
find "$BACKUP_ROOT/weekly"  -type f -name 'powertradefx-*.dump' -mtime +56 -delete
find "$BACKUP_ROOT/monthly" -type f -name 'powertradefx-*.dump' -mtime +365 -delete
log "rotation done"
