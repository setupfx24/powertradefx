...
function StatCard({ label, value, icon: Icon, tone = 'default' }: {
  ...
  const toneClasses = {
    default: { bg: 'bg-bg-tertiary', icon: 'text-text-primary' },
  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-card p-4">
  ...
  return (
    <li className="flex items-start gap-3 text-left">
      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/15">
        <CheckCircle2 size={13} className="text-emerald-400" strokeWidth={2.6} />
      </span>
  ...
  return (
    <DashboardShell>
      <div className="space-y-6">
        {/* Hero header */}
        <div className="relative overflow-hidden rounded-2xl border border-border-primary bg-card p-6 sm:p-8">
          <div
            className="pointer-events-none absolute inset-0 bg-gradient-to-br from-accent-soft via-white to-white"
            aria-hidden
          />
        ...
        {/* Tabs strip */}
        <div className="rounded-2xl border border-border-primary bg-card">
            ...
            {TABS.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <span
                      className="pointer-events-none absolute inset-x-3 -bottom-px h-[3px] rounded-t-full bg-accent"
                      aria-hidden
                    />
                  )}
                </button>
      ...
      {dashboard?.referral_link && (
        <div className="rounded-2xl border border-border-primary bg-card p-5">
          <div className="flex items-center gap-2 mb-3">
            <Share2 size={15} className="text-accent" strokeWidth={2.4} />
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-border-primary bg-bg-tertiary px-3 py-2.5 text-xs font-mono text-text-primary focus:outline-none"
            />
            <button
              type="button"
              onClick={() => { navigator.clipboard.writeText(dashboard.referral_link); toast.success('Copied!'); }}
              className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-accent px-3.5 py-2.5 text-xs font-semibold text-white hover:bg-accent-hover transition-colors"
            >
              <Copy size={13} />
              Copy
            </button>
          ...
          <p className="mt-2 text-xs text-text-tertiary">
            Code: <span className="font-mono font-bold text-accent">{dashboard.referral_code}</span>
          </p>
        </div>
      )}
      ...
      {commissions.length > 0 && (
        <DataTable
          ...
          rows={commissions.map((c: any) => [
            ...
            <span key={`t-${c.id}`} className="text-text-tertiary capitalize">{c.commission_type?.replace('_', ' ')}</span>,
            <span key={`l-${c.id}`} className="text-text-tertiary">L{c.mlm_level}</span>,
            <span key={`a-${c.id}`} className="font-mono tabular-nums text-emerald-400">${fmt(c.amount || 0)}</span>,
            (
              <span
                key={`st-${c.id}`}
                className={clsx(
                  'inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold',
                  c.status === 'paid' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-300',
                )}
              >
                {c.status}
              </span>
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-border-primary bg-card p-5">
          ...
          <div className="flex items-center gap-3">
            <span className="rounded-lg bg-bg-tertiary px-4 py-2.5 text-lg font-bold font-mono text-accent">
              {dashboard.referral_code}
            </span>
            <button
              type="button"
              onClick={() => { navigator.clipboard.writeText(dashboard.referral_code); toast.success('Copied!'); }}
              className="inline-flex items-center gap-1.5 rounded-lg bg-accent px-3.5 py-2.5 text-xs font-semibold text-white hover:bg-accent-hover transition-colors"
            >
              <Copy size={13} />
              Copy
            </button>
            ...
            rows={dashboard.clients.map((c: any) => [
              ...
              (
                <span
                  ...
                  className={clsx(
                    'inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold',
                    c.status === 'active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-bg-tertiary text-text-secondary',
                  )}
                >
                  {c.status}
                </span>
      ...
      extra={(
        <div className="text-left">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border-primary bg-card px-4 py-2.5 text-sm text-text-primary placeholder:text-text-tertiary outline-none focus:border-accent focus:ring-2 focus:ring-accent/15"
          />
        </div>
      )}
    />
...
function NetworkTab() {
  ...
  if (!tree) {
    return (
      <div className="rounded-2xl border border-dashed border-border-primary bg-bg-tertiary/60 py-16 px-6 text-center max-w-xl mx-auto">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-card border border-border-primary mb-3">
          <Network size={20} className="text-text-tertiary" />
        </div>
        <p className="text-sm font-semibold text-text-primary">No network yet</p>
        <p className="mt-1 text-xs text-text-tertiary">You need to be an approved IB to see your network.</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-border-primary bg-card p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Network size={16} className="text-accent" strokeWidth={2.4} />
            <h3 className="text-sm font-semibold text-text-primary">Your MLM Network</h3>
          </div>
          <span className="text-xs text-text-tertiary">{tree.total_nodes || 0} members</span>
        </div>
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs">
          ...
          <span className="text-text-tertiary">
            Total Earned: <span className="font-mono font-bold text-emerald-400">${fmt(tree.root?.total_earned || 0)}</span>
          </span>
        </div>
      </div>
      ...
      {tree.tree?.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-border-primary bg-bg-tertiary/60 py-10 px-6 text-center text-sm text-text-tertiary">
  ...
  return (
    <div style={{ marginLeft: depth * 18 }}>
      ...
      >
        ...
        <span className="rounded-md bg-accent-soft px-1.5 py-0.5 text-[10px] font-mono font-bold text-accent">L{node.depth}</span>
        <span className="ml-auto font-mono tabular-nums text-text-tertiary">${fmt(node.total_earned || 0)}</span>
        {!node.is_active && (
          <span className="rounded-full bg-red-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-red-400">inactive</span>
        )}
      </button>
...
function PendingCard({ message }: { message: string }) {
  return (
    <div className="mx-auto max-w-lg rounded-2xl border border-amber-200 bg-amber-500/10/50 p-8 text-center">
      <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-amber-500/15 mb-3">
    ...
    <div className="mx-auto max-w-2xl">
      <div className="overflow-hidden rounded-2xl border border-border-primary bg-card">
        <div className="relative px-6 sm:px-8 pt-8 pb-6 text-center">
          <div
            className="pointer-events-none absolute inset-0 bg-gradient-to-b from-accent-soft/60 to-transparent"
            aria-hidden
          />
          <div className="relative">
            <span className="inline-flex items-center rounded-full bg-accent-soft px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-accent">
              {eyebrow}
            </span>
        ...
        <div className="border-t border-border-primary px-6 sm:px-8 py-6">
          ...
          <div className="mt-6 flex justify-center">
            <button
              ...
              disabled={disabled}
              className={clsx(
                'inline-flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white transition-colors shadow-[0_2px_8px_rgb(var(--accent-rgb) / 0.25)] min-w-[200px]',
                disabled
    ...
    <div className="overflow-hidden rounded-2xl border border-border-primary bg-card">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map((cells, ri) => (
              <tr key={ri} className="border-b border-[#F5F5F5] last:border-b-0 hover:bg-bg-secondary/60">
                {cells.map((cell, ci) => (
                  <td
                    key={ci}
                    className={clsx(
